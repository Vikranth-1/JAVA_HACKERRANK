...
public class Solution {
    ...
    private static Tree buildTree(TreeNode parent, int parentIndex) {
        Set<Integer> childrenIndices = adjacencyList.get(parentIndex);
        if (childrenIndices != null) {
            for (int childIndex : childrenIndices) {
                adjacencyList.get(childIndex).remove(parentIndex);
                
                if (adjacencyList.get(childIndex).isEmpty()) {
                    parent.addChild(new TreeLeaf(values[childIndex], colors[childIndex], parent.getDepth() + 1));
                } else {
                    TreeNode childNode = new TreeNode(values[childIndex], colors[childIndex], parent.getDepth() + 1);
                    parent.addChild(childNode);
                    buildTree(childNode, childIndex);
                }
            }
        }
        return parent;
